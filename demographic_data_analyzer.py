...
def calculate_demographic_data(print_data=True):
    ...
    education_total = len(df['education'])
    percentage_bachelors = round(len(df[Bachelor])* 100/ education_total, 1)

    # What percentage of people with advanced education (`Bachelors`, `Masters`, or `Doctorate`) make more than 50K?
    advanced = df[df['education'].isin(['Bachelors', 'Masters', 'Doctorate'])]
    advanced_education = round(len(advanced[advanced['salary'] == '>50K']) * 100 / len(advanced), 1)
    # What percentage of people without advanced education make more than 50K?
    without_advanced = df[~df['education'].isin(['Bachelors', 'Masters', 'Doctorate'])]
    without_advanced_education = round(len(without_advanced[without_advanced['salary'] == '>50K']) * 100 / len(without_advanced), 1)
    # with and without `Bachelors`, `Masters`, or `Doctorate`
    # 
    # percentage with salary >50K
    higher_education_rich = advanced_education
    lower_education_rich = without_advanced_education
